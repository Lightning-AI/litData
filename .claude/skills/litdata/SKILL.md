...
| Topic          | Remember                                                                                                                                                                                                                                                          |
| -------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Raw files**  | `StreamingRawDataset`: raw `bytes` as-is; group/order via `setup`; async + batched downloads, retries; torch `DataLoader` — `#stream-raw` / §10. Optimized is still faster; raw is **not too far behind** with full per-file control. Default `max_prefetch=16`; `range_parallel_threshold=0` |
