...
| Topic          | Remember                                                                                                                                                                                                                                                          |
| -------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Raw files**  | `StreamingRawDataset`: raw `bytes`, fully async + batched downloads, retries; torch `DataLoader` — `#stream-raw` / `using-litdata.md` §10. Default `max_prefetch=16`; `range_parallel_threshold=0` (ranged opt-in). See README Before vs After long-window matrix |
...
| Optimize       | `if __name__ == "__main__"`; exactly one of `chunk_bytes` \| `chunk_size`. Default **64MB**; multi‑MB samples → consider **256–512MB** (more intra-chunk shuffle; slower download). Expert range, not a published sweep — README `#faq-chunk-shuffle`             |
