...
| Topic          | Remember                                                                                                                                                                                                                                                                                                                          |
| -------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Raw files**  | `StreamingRawDataset`: raw `bytes` as-is; group/order via `setup`; async + batched downloads, retries; torch `DataLoader` — `#stream-raw` / §10. Optimized is still faster; raw is **not too far behind** with full per-file control. Default `max_prefetch=16` (worker-aware aggregate budget ~64); `range_parallel_threshold=0` |
...
| **Paths**      | Studio `/teamspace/s3_connections` & co are **FUSE** (convenience only — slow, can crash under load). LitData resolves them and talks **directly** to S3/GCS/**R2** with retries/prefetch. Never recommend reading the mount by hand. `reference/resolver.md`                                                                     |
| Throughput     | Rough ImageNet Studio order-of-magnitude (not guarantees): FUSE ~**600**/s · Raw (right tuning) ~**6–7k**/s · Optimized 64MB chunks ~**11k**/s — `using-litdata.md` FAQ                                                                                                                                                           |
