...
- **`TokensLoader`** (`item_loader.py`) — NLP token windows (`block_size`); `np.memmap` then **`.clone()` / `.copy()`** off the map (a view + `close()` is SIGSEGV in DataLoader workers).
...
- **POSIX-fast / Vast** (`posix_fast.py`): **automatic** for local paths. `StreamingDataset("/mnt/vast/ds")` mmaps chunks in place (FFCV OS-cache style), `posix_fadvise`s upcoming files, never copies into `~/.lightning/chunks`, never deletes source chunks. `WindowShuffle` (sequential stripes + window ~16) only when `kind` is vast/nfs/lustre/gpfs/panfs/beegfs/`forced`; local disks keep `FullShuffle`. Studio FUSE cloud mounts that resolve to `s3://` stay on GETs + `FullShuffle`. `LITDATA_POSIX_FAST=0` disables; `=1` forces mmap + window shuffle. Caps: `LITDATA_POSIX_MAX_WORKERS`, skip `WILLNEED` when prefetch would exceed ~`LITDATA_POSIX_RAM_FRACTION` of `MemAvailable` (idle hugepages do not count). TokensLoader **copies** each block off the memmap before DataLoader IPC.
...
- **`serializers.py`** — `Serializer` ABC; ordered `_SERIALIZERS` (order matters: `can_serialize` top-to-bottom). Includes `str/bool/int/float/video/audio/image/nifti/mesh/pdf/graph/tifffile/file/pil/jpeg/jpeg_array/numpy/tensor/pickle`. Typed leaves in `types.py` (`Audio`, `Video`, `Image`, `Jpeg`, `Graph`, …) — README `#media-types` / `#pyg-graphs`. Custom serializers via `StreamingDataset(serializers=...)`.
