...
| `LITDATA_POSIX_SHUFFLE_WINDOW`         | `16`                                        | With POSIX-fast + `shuffle=True`: after sequential chunk assignment, each worker permutes only inside this many upcoming chunks. `1` = sequential chunk order (in-chunk shuffle still on). Object URLs keep global `FullShuffle`. |
| `MAX_CACHE_SIZE`                       | ctor `max_cache_size` (default `"100GB"`)   | Env override for eviction budget (bytes / parsed like ctor)                                                                                                                                                                       |
