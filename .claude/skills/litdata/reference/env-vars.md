...
| `LITDATA_POSIX_SHUFFLE_WINDOW`         | `16`                                        | With POSIX-fast + `shuffle=True`: sequential chunk stripes, then a window permute per worker **and** inside each chunk so item reads stay on one mmap page. `1` = sequential. Object URLs keep `FullShuffle`.                     |
