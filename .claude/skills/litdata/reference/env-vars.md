...
| `LITDATA_POSIX_WILLNEED`               | auto                                                           | `0` skips `POSIX_FADV_WILLNEED` / `MADV_WILLNEED`. Also skipped automatically when workers × keep × chunk would exceed ~half `MemAvailable` — including **remote** downloaded cache files, not only POSIX-fast mmap.            |
| `LITDATA_POSIX_RAM_FRACTION`           | `0.5`                                                          | Fallback budget when `MemTotal` is unknown: fraction of `MemAvailable` for WILLNEED / worker-cap / prefetch.                                                                                                                    |
| `LITDATA_RAM_CEILING`                  | `0.95`                                                         | Max used fraction of `MemTotal`. LitData shrinks prefetch/workers, skips `WILLNEED`, and throttles decode/yield so used RAM stays around 95% (sshd).                                                                            |
| `LITDATA_RAM_SOFT_MARGIN`              | `0.20`                                                         | Start slowing this far below the ceiling (default: throttle from 75% → 95%). Quadratic sleep + shrinking `max_pre_download`.                                                                                                    |
| `LITDATA_RAM_WAIT_TIMEOUT`             | `120`                                                          | Max seconds to block once used RAM is at the ceiling. On timeout, LitData raises rather than allocating beyond the ceiling. `0` disables waits.                                                                                 |
