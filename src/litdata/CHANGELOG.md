...
- Elastic mid-epoch resume: new epochs use a topology-independent canonical sample order. Changing `num_workers` or `world_size` restripes the remaining `sample_in_epoch` suffix (no duplicates). v1 checkpoints with the same topology keep prefix replay. `force_override_state_dict` does not override worker count.
