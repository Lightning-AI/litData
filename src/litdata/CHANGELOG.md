...
- Elastic mid-epoch resume: changing `num_workers` or `world_size` restripes the remaining canonical sample stream (`sample_in_epoch`). Same topology stays on prefix replay. `num_canonical_nodes` freezes shuffle buckets. WindowShuffle uses chunk granularity.
