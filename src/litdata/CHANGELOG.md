...
- Remote streaming now backs off before host RAM hits ~100% (SSH hang on 1× H100). Default ``LITDATA_RAM_CEILING=0.95``: reserve half of the SSH headroom for queued decoded batches; cap `max_pre_download` / `prefetch_factor`, and **throttle early** across the 75–95% RAM band (``LITDATA_RAM_SOFT_MARGIN=0.20``) so used RAM stays near its target instead of slamming into the ceiling and stalling. RAM accounting uses the lower of host memory and a finite cgroup v1/v2 limit, preventing a container from treating unaddressable host RAM as capacity. In that band it skips `WILLNEED` and evicts consumed chunks, preventing page-cache growth. The user-selected worker count is preserved; once RAM enters the band, completed worker tasks are not refilled until the queue drains, then one task is drip-fed at a time. At the ceiling, hold until pressure drops to the middle of the band; after ``LITDATA_RAM_WAIT_TIMEOUT=120`` seconds, fail closed rather than allocating further. Reproduce with `scripts/bench/bench_ram_pressure.py` on `/teamspace/lightning_storage/litdata-r2`.
