...
def test_set_and_get_items_int_index():
    provider = WorkerItemProvider([[1, 2], [3, 4]], num_downloaders=2, num_workers=2)
...
def test_invalid_index_type():
    with pytest.raises(AssertionError, match="Expected"):
        provider = WorkerItemProvider([[1, 2], [3, 4]], num_downloaders=1, num_workers=1)
    provider = WorkerItemProvider([[1, 2], [3, 4]], num_downloaders=1, num_workers=2)
...
def test_prepare_ready_to_use_queue(use_shared):
    provider = WorkerItemProvider([[1, 2, 3], [4, 5, 6]], num_downloaders=2, num_workers=2)
    ...
    for worker_index in range(2):
        queue = (
            provider.ready_to_process_shared_queue if use_shared else provider.ready_to_process_item_queue[worker_index]
        )
        contents = []
        none_count = 0
        while none_count < 2:
