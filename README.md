...
Same `seed` and `shuffle` are required. **`num_workers` and `world_size` may change**: LitData drops a global `sample_in_epoch` prefix and restripes the rest (never duplicates remaining IDs). For a matching loss curve keep **global batch size** (`world_size * batch_size`) constant and DDP ranks in lockstep. Optional `num_canonical_nodes` (default: first-run `world_size`) freezes shuffle buckets like Mosaic Streaming. POSIX `WindowShuffle` resumes whole remaining chunks.
